...
async function refreshBicycleParking() {
	await cache.put(
		'bicycle_parking',
		await dataFetcher.fetchBicycleParking(),
		OPENSTREETMAP_REFRESH,
		refreshBicycleParking
	);
...
async function refreshVilloStation() {
	await cache.put(
		'villo_station',
		await dataFetcher.fetchVilloStations(),
		VILLO_STATION_REFRESH,
		refreshVilloStation
	);
...
async function refreshCompressedAir() {
	await cache.put(
		'compressed_air',
		await dataFetcher.fetchAirPumps(),
		OPENSTREETMAP_REFRESH,
		refreshCompressedAir
	);
...
async function refreshBicycleRepairStations() {
	await cache.put(
		...
		await dataFetcher.fetchBicycleRepairStation(),
		OPENSTREETMAP_REFRESH,
		refreshBicycleRepairStations
	);
...
async function refreshBicycleShop() {
	await cache.put(
		'bicycle_shop',
		await dataFetcher.fetchBicycleShop(),
		OPENSTREETMAP_REFRESH,
		refreshBicycleShop
	);
...
async function refreshDrinkingWater() {
	await cache.put(
		'drinking_water',
		await dataFetcher.fetchDrinkingWater(),
		OPENSTREETMAP_REFRESH,
		refreshDrinkingWater
	);
...
async function refreshOfficialEvents() {
	cache.put(
		'official_events',
		await dataFetcher.fetchOfficialEvents(),
