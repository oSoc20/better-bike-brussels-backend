   ...
   */
	filter(center_latitude, center_longitude, radius, max_answers) {
		...
		case 'FeatureCollection': {
			let list = this.geoJson.features
				.map((item) => {
					let sub_filter = new GeoFilter(item);
					return sub_filter.filter(
						center_latitude,
						center_longitude,
						radius,
						max_answers
		...
		case 'Feature': {
			...
			if (
				Distance.between(client_center, feature_pos) < Distance(radius + 'm')
			) {
				this.geoJson.properties.dist = Distance.between(
					client_center,
					feature_pos
				).human_readable();
				this.geoJson.radians = Distance.between(
					client_center,
					feature_pos
				).radians;
